...
from common import MPCConfigDYNBase as MPCConfigDYN, DrawDebug, ground_truth_pacejka, make_render_callback, make_log_dict, apply_slip_friction
...
def main():
    ...
    with open(os.path.join('results', f'scale{dyn_config.scale}_TK{dyn_config.TK}_log_{map_name}_full_Vinit_{v_x_init}_c{dyn_config.q_contour}_l{dyn_config.q_lag}_p{dyn_config.q_theta}_friction{constant_friction}_weight{weight_slip}_slip_{time_slip_start}_{time_slip_end}_{time_slip_start_2}_{time_slip_end_2}_{time_slip_start_3}_{time_slip_end_3}_non'), 'w') as f:
        json.dump(log, f)
