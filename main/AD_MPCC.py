...
from common import (
    ...
    make_render_callback, make_log_dict, apply_slip_friction, compute_stage_weights,
)
...
def main():
    ...
    obs, step_reward, done, info = env.reset( np.array([[waypoints[start_point, 1],
                                                         waypoints[start_point, 2],
                                                         (waypoints[start_point, 3]+ np.pi) % (2*np.pi) - np.pi,
                                                         0.0,
                                                         v_x_init,
                                                         0.0,
                                                         0.0]]))
    ...
    while not done:
        ...
        if len(x_hist) > T_horizon+10:
            ...
            param = Pacejka_func.find_param(np.asarray(x_hist[-T_horizon:]), np.asarray(u_hist[-T_horizon:]))
            BR = param[0]
            CR = param[1]
            DR = param[2]
            BF = param[3]
        ...
        u, mpc_ref_path_x, mpc_ref_path_y, mpc_pred_x, mpc_pred_y, done = planner_dyn_mpc.plan(vehicle_state, param, weights)
        end_compute_time = time.time()
        print("Compute time for current step:", round(end_compute_time - start_compute_time, 3), "seconds")
        u_hist.append(u)
    ...
    with open(os.path.join('results', f'ad_mpcc_scale{dyn_config.scale}_Tk{dyn_config.TK}_log_{map_name}_full_Vinit_{v_x_init}_c{dyn_config.q_contour}_l{dyn_config.q_lag}_p{dyn_config.q_theta}_friction{constant_friction}_weight{weight_slip}_slip_{time_slip_start}_{time_slip_end}_{time_slip_start_2}_{time_slip_end_2}_{time_slip_start_3}_{time_slip_end_3}'), 'w') as f:
        json.dump(log, f)
